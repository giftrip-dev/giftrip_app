...
/// 목업 장바구니 데이터
final List<CartItemModel> mockCartList = [
  // 숙박 2개
  for (int i = 0; i < 2; i++)
    CartItemModel(
      ...
      title: '숙박 상품상품상품상품상품상품상품상품상품 ${i + 1}',
      ...
      quantity: 1,
      addedAt: DateTime.now().subtract(Duration(days: i + 1)),
      tags: [],
    ),
  // 체험 2개
  for (int i = 0; i < 2; i++)
    CartItemModel(
      ...
      quantity: 1,
      addedAt: DateTime.now().subtract(Duration(days: i + 2)),
      tags: ['특가상품'],
    ),
  // 상품 3개
  for (int i = 0; i < 3; i++)
    CartItemModel(
      ...
      quantity: 1,
      addedAt: DateTime.now().subtract(Duration(days: i + 3)),
      tags: [],
    ),
  // 체험단 1개
  CartItemModel(
